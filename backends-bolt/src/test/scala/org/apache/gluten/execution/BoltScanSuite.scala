...
import org.apache.gluten.backendsapi.bolt.BoltBackendSettings
import org.apache.gluten.benchmarks.RandomParquetDataGenerator
import org.apache.gluten.config.{BoltConfig, GlutenConfig}
import org.apache.gluten.utils.BoltFileSystemValidationJniWrapper
...
class BoltScanSuite extends BoltWholeStageTransformerSuite {
  ...
  test("Test file scheme validation") {
    ...
    val filteredRootPath =
      BoltBackendSettings.distinctRootPaths(
  ...
  test("partition column reuses a data column name (SPARK-18108)") {
    // File contains both 'a' (BIGINT) and 'b' (DOUBLE); the path 'a=1/'
    // makes Spark infer 'a' as an INT partition column. Spark's
    // semantics: partition value wins, file's 'a' is not read. Two
    // assertions: partition discovery gives Row(1, 2.0); basePath
    // suppresses inference and surfaces the file's BIGINT.
    withTempDir {
      dir =>
        val path = dir.getCanonicalPath
        spark
        ...
        // Sanity: without the Bolt scan path the assertion above is
        // satisfied by vanilla Spark and the test loses its value.
        assert(
        ...
        val viaBasePath =
          spark.read.option("basePath", s"$path/a=1").parquet(s"$path/a=1")
        checkAnswer(viaBasePath, Seq(Row(42L, 2.0)))
    }
  }
